...
public class ContentTest extends KatelloCliLongrunBase {
	...
	public void setUp(){
		...
		if(!findSyncedRhelToUse()){
			res = new KatelloOrg(base_org_name, null).cli_create();
			Assert.assertTrue(res.getExitCode().intValue() == 0, "Check - exit.Code");
			KatelloUtils.scpOnClient("data/"+manifestZip, "/tmp");
			res = new KatelloProvider(KatelloProvider.PROVIDER_REDHAT, base_org_name, null, null).import_manifest("/tmp/"+manifestZip, new Boolean(true));
		...
		res = new KatelloEnvironment(envTesting, null, base_org_name, KatelloEnvironment.LIBRARY).cli_create();
		Assert.assertTrue(res.getExitCode().intValue() == 0, "Check - exit.Code");
	}
	...
	public void test_syncAndPromoteRhel6(){
		...
		// promote
		KatelloContentDefinition cd = new KatelloContentDefinition("cd"+uid, null, base_org_name, null);
		res = cd.create();
		Assert.assertTrue(res.getExitCode().intValue() == 0, "Check - exit.Code");
		...
		Assert.assertTrue(res.getExitCode().intValue() == 0, "Check - exit.Code");
		KatelloContentView cont = new KatelloContentView(contView, base_org_name);
	...
	public void test_productStatusSynced(){
		res = new KatelloProduct(KatelloProduct.RHEL_SERVER, base_org_name, KatelloProvider.PROVIDER_REDHAT, null, null, null,null, null).status();
		Assert.assertTrue(res.getExitCode().intValue() == 0, "Check - exit.Code");
	...
	public void test_regConsumer(){
		KatelloSystem sys = new KatelloSystem(sysName, base_org_name, envTesting+"/"+contView);
