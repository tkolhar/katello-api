...
public class PromoteChangeset extends KatelloCliTestBase {
	...
	public void setUp(){
		...
		KatelloEnvironment env = new KatelloEnvironment(this.cli_worker, this.env1, null, this.org, KatelloEnvironment.LIBRARY);
		res = env.cli_create();
		Assert.assertTrue(res.getExitCode().intValue()==0, "Check - return code");
		env = new KatelloEnvironment(this.cli_worker, this.env2, null, this.org, this.env1);
		res = env.cli_create();
		Assert.assertTrue(res.getExitCode().intValue()==0, "Check - return code");
		env = new KatelloEnvironment(this.cli_worker, this.env3, null, this.org, this.env2);
		res = env.cli_create();
		Assert.assertTrue(res.getExitCode().intValue()==0, "Check - return code");
...
//		Assert.assertTrue(res.getExitCode().intValue()==0, "Check - return code");
	...
	public void test_promotePackageToDEV(){
		KatelloUtils.promotePackagesToEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {"lion"}, this.env1);
		
		KatelloPackage pack = new KatelloPackage(cli_worker, null, null, this.org, this.product, this.repo, this.env1);
	...
	public void test_promoteErrataToDEV(){
		KatelloUtils.promoteErratasToEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {PromoteErrata.ERRATA_ZOO_SEA}, this.env1);

		KatelloErrata err = new KatelloErrata(cli_worker, null, this.org, this.product, this.repo, this.env1);
	...
	public void test_promotePackageToGA(){
		KatelloUtils.promotePackagesToEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {"lion"}, this.env2);

		KatelloPackage pack = new KatelloPackage(cli_worker, null, null, this.org, this.product, this.repo, this.env2);
		SSHCommandResult res = pack.cli_list();
		Assert.assertTrue(getOutput(res).replaceAll("\n", "").contains("lion"), "Check - package list output");
	}
	...
	public void test_promoteErrataToGA(){
		KatelloUtils.promoteErratasToEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {PromoteErrata.ERRATA_ZOO_SEA}, this.env2);
		
		KatelloErrata err = new KatelloErrata(cli_worker, null, this.org, this.product, this.repo, this.env2);
		SSHCommandResult res = err.cli_list();
		Assert.assertTrue(getOutput(res).replaceAll("\n", "").contains(PromoteErrata.ERRATA_ZOO_SEA), "Check - errata list output");
	}
	...
	public void test_deletePackageFromGA(){
		KatelloUtils.removePackagesFromEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {"lion"}, this.env2);
	...
	public void test_deleteErrataFromGA(){
		KatelloUtils.removeErratasFromEnvironment(cli_worker, this.org, this.product, this.repo, new String[] {PromoteErrata.ERRATA_ZOO_SEA}, this.env2);
