...
public class ChangesetProductDelete extends KatelloCliTestScript {
	
	protected static Logger log = Logger.getLogger(ChangesetProductDelete.class.getName());
	...
	@Test(description = "Create changeset of deletion type," +
			" then add already promoted product to changeset and apply it," +
			" verify that product does not exist in environment anymore")
	public void test_deletionChangesetRemoveRepo() {
		...
		exec_result = prod.cli_list(env_name);
		Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");
		Assert.assertTrue(getOutput(exec_result).contains(product_name));
		
		KatelloUtils.removeProductFromEnvironment(org_name, product_name, env_name);

		exec_result = prod.cli_list(env_name);
		Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");
		Assert.assertFalse(getOutput(exec_result).contains(product_name));
