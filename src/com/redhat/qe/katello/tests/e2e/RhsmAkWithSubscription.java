...
public class RhsmAkWithSubscription extends KatelloCliTestBase{
	...
	public void setUp(){
		exec_result = new KatelloOrg(this.cli_worker, org, null).cli_create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloEnvironment(this.cli_worker, env, null, org, KatelloEnvironment.LIBRARY).cli_create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
	}
	
	@Test(description="prepare and sync repo")
	public void test_syncRepo(){
		exec_result = new KatelloProvider(this.cli_worker, provider, org, null, null).create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloProduct(this.cli_worker, product, org, provider, null, null, null, null, null).create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloRepo(this.cli_worker, repo, org, product, REPO_HHOVSEPY_ZOO4, null, null).create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloProvider(this.cli_worker, provider, org, null, null).synchronize();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
	}
	...
	public void test_promoteViaContentView(){
		KatelloContentDefinition _contDef = new KatelloContentDefinition(cli_worker, contentDef, null, org, null);
		...
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloContentView(cli_worker, contentView, org).promote_view(env);
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
	}
	...
	public void test_prepareAk(){
		KatelloActivationKey _ak = new KatelloActivationKey(this.cli_worker, org,env,ak,null,"10",contentView);
		exec_result = _ak.create();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
		exec_result = new KatelloOrg(this.cli_worker, org, null).subscriptions();
		Assert.assertTrue(exec_result.getExitCode().intValue()==0, "Check - exitCode");
	...
	public void test_registerAndConsumeRpm(){
		rhsm_clean();
		KatelloSystem _sys = new KatelloSystem(this.cli_worker, system, org, null);
