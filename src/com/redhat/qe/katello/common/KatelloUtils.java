...
public class KatelloUtils implements KatelloConstants {
	...
	private static void promoteToEnvironment(String org_name, String product_name, String repo_name, String[] package_names, String[] erratas, String env_name, boolean promote) {
		...
		Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");
		if (erratas != null) {
			exec_result = filter.add_rule_errata(( promote ? KatelloContentFilter.TYPE_INCLUDES : KatelloContentFilter.TYPE_EXCLUDES), erratas);
			Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");
		}
		if (package_names != null) {
			String [] packages = new String[package_names.length];
			for (int i = 0; i < package_names.length; i ++) {
				packages[i] = "\\\"name\\\" : \\\"" + package_names[i] + "\\\"";
			}
			exec_result = filter.add_rule_package(( promote ? KatelloContentFilter.TYPE_INCLUDES : KatelloContentFilter.TYPE_EXCLUDES), packages);
			Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");		
		}
		exec_result = condef.publish(pubview_name, pubview_name,"Publish Content");
		Assert.assertTrue(exec_result.getExitCode() == 0, "Check - return code");		
